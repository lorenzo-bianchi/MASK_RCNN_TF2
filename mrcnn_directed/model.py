...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        prop = np.zeros(shape=(1, 1000, 4), dtype=np.float32)
        prop[0, 0, :] = np.array([-0.02211869, -0.01105934,  0.02114117,  0.01008183])
...
# ********** Directed Mask R-CNN **********
class ProposalLayerDirected(KE.Layer):
    """Receives anchor scores and selects a subset to pass as proposals
    to the second stage. Filtering is done based on anchor scores and
    non-max suppression to remove overlaps. It also applies bounding
    box refinement deltas to anchors.
    ...
    def __init__(self, proposal_count, nms_threshold, config=None, **kwargs):
        super(ProposalLayerDirected, self).__init__(**kwargs)
        self.config = config
        self.proposal_count = proposal_count
        self.nms_threshold = nms_threshold
    ...
    def call(self, inputs):
        ...
        proposals = tf.convert_to_tensor(self.config.REGION_PROPOSALS)
...
#            self.config.NAME.lower(), now))
        self.log_dir = "D:\\Object Detection\\Kangaro - Transfer Learning\\logdir\\train"
